  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Link href="/events" className="btn-primary"><Plus className="h-4 w-4" /> New Event</Link>}
      />
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map(ev => (
            <Link href={`/events/${ev.id}`} key={ev.id} className="card p-5 hover:border-accent-400 hover:shadow-md">
