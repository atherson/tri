...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card title="Top Synchronization Alerts" actions={<Link href="/alerts" className="text-xs font-medium text-accent-600 hover:underline">View all</Link>}>
          {allAlerts.length === 0 ? (
            <EmptyState icon={<CheckCircle2 className="h-8 w-8" />} title="No alerts" />
          ) : (
            <div className="space-y-3">
              {allAlerts.slice(0, 5).map(alert => (
                <Link href="/alerts" key={alert.id} className="flex items-start gap-3 rounded-lg border p-3 hover:bg-slate-50 dark:hover:bg-navy-800">
        ...
        <Card title="Recent Events" actions={<Link href="/events" className="text-xs font-medium text-accent-600 hover:underline">View all</Link>}>
          <div className="space-y-3">
            {events.slice(0, 5).map(ev => {
              const cs = costSummaries.get(ev.id)
              return (
                <Link href={`/events/${ev.id}`} key={ev.id} className="flex items-center justify-between gap-3 rounded-lg border p-3 hover:bg-slate-50 dark:hover:bg-navy-800">
                  <div className="flex-1">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <Link href="/events" className="card flex flex-col items-center gap-2 p-4 hover:border-accent-400 hover:shadow-md">
        ...
        <Link href="/contracts" className="card flex flex-col items-center gap-2 p-4 hover:border-accent-400 hover:shadow-md">
        ...
        <Link href="/equipment" className="card flex flex-col items-center gap-2 p-4 hover:border-accent-400 hover:shadow-md">
        ...
        <Link href="/movements" className="card flex flex-col items-center gap-2 p-4 hover:border-accent-400 hover:shadow-md">
