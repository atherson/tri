...
export default function Contracts() {
  ...
  return (
    <div>
      <PageHeader title="Rental Contracts" subtitle={`${contracts.length} contract${contracts.length !== 1 ? 's' : ''} total`} />
              ...
              {filtered.map(c => (
                <tr key={c.id} className="border-b hover:bg-slate-50 dark:hover:bg-navy-800/50">
                  <td className="py-3 pr-4">
                    <Link href={`/contracts/${c.id}`} className="font-medium text-accent-600 hover:underline">{c.contract_number}</Link>
                  ...
                  <td className="py-3 pr-4 text-xs text-slate-500 dark:text-slate-400">{formatDate(c.contract_start)} → {formatDate(c.contract_end)}</td>
